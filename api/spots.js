export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(`supabase read failed (${r.status}): ${body.slice(0, 200)}`);
      }
      const data = await r.json();
      return res.status(200).json(data);
    } catch (err) {
      return res.status(500).json({ error: `Could not load community spots: ${err.message || err}` });
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!r.ok) {
        const body = await r.text();
        throw new Error(`supabase insert failed (${r.status}): ${body.slice(0, 300)}`);
      }
      const data = await r.json();
      return res.status(201).json(data);
    } catch (err) {
      return res.status(500).json({ error: `Could not submit spot: ${err.message || err}` });
    }
  }
