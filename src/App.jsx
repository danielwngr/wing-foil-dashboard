            ...
            return (
              ...
              >
                ...
                <div className="mono" style={{ fontSize: 9.5, lineHeight: 1.45, textAlign: 'center' }}>
                  {goodSpots.length ? (
                    goodSpots.map((g) => (
                      <div key={g.name} style={{ color: active ? COLORS.paper : COLORS.go, fontWeight: 600 }}>
                        {g.name} <span style={{ opacity: 0.75, fontWeight: 400 }}>({g.window})</span>
                      </div>
                    ))
                  ) : (
                    <span style={{ color: active ? 'rgba(237,230,214,0.5)' : COLORS.paperLine }}>{'\u2014'}</span>
                  )}
                </div>
              </button>
            );
            ...
            (() => {
              ...
              return (
                <>
                  ...
                  {/* Wind direction arrows, colored by whether they match the spot's working sectors */}
                  <div style={{ display: 'flex', gap: 2, minWidth: 620, marginTop: 4 }}>
                    ...
                    {activeScore.scored.map((h) => (
                      <div key={h.hour} style={{ flex: 1, display: 'flex', justifyContent: 'center' }} title={`from ${h.dir}\u00b0`}>
                        <ArrowUp
                          size={13}
                          color={directionColor(h.dir, activeSpot.sectors)}
                  ...
                  <div style={{ display: 'flex', gap: 6, minWidth: 620, marginTop: 8 }}>
                    <div style={{ position: 'relative', width: AXIS_WIDTH, flexShrink: 0, height: BAR_AREA_HEIGHT }}>
                      {ticks.map((v) => (
                        <span
                          key={v}
                          className="mono"
                          style={{ position: 'absolute', right: 4, bottom: `${(v / scaleMax) * 100}%`, transform: 'translateY(50%)', fontSize: 9, color: COLORS.inkSoft }}
                        >
                          {v}
                        </span>
                      ))}
                    ...
                    <div style={{ position: 'relative', flex: 1, height: BAR_AREA_HEIGHT }}>
                      {ticks.map((v) => (
                        <div
                          key={v}
                          style={{ position: 'absolute', left: 0, right: 0, bottom: `${(v / scaleMax) * 100}%`, borderTop: `1px dotted ${COLORS.paperLine}` }}
                        />
                      ))}
                            ...
                            <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end', height: '100%' }}>
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                                ...
                                <div
                                  ...
                                  style={{ width: 10, height: `${Math.min(100, (h.speed / scaleMax) * 100)}%`, background: scoreColor(h.score) }}
                                />
                              </div>
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                                ...
                                <div
                                  ...
                                  style={{ width: 10, height: `${Math.min(100, (h.gust / scaleMax) * 100)}%`, background: scoreColor(h.score), opacity: 0.4 }}
                                />
                              </div>
                            </div>
                          </div>
