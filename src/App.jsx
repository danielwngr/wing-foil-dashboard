...
export default function App() {
  ...
  // appearing in the public nav. #manage?token=... works the same way, for
  // the private per-subscriber link included in every notification email.
  const hash = typeof window !== 'undefined' ? window.location.hash : '';
  const isAdminRoute = hash === '#admin';
  const manageMatch = hash.match(/^#manage\?token=(.+)$/);
  const manageToken = manageMatch ? decodeURIComponent(manageMatch[1]) : null;
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: COLORS.paper, minHeight: '100vh', color: COLORS.ink }}>
      ...
      {manageToken ? (
        <ManagePage token={manageToken} spots={allSpots} />
      ) : isAdminRoute ? (
