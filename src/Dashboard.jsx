          ...
          {spots.map((s) => {
            ...
            return (
              <CircleMarker
                key={s.id}
                ref={(layer) => {
                  // Leaflet stacks CircleMarkers by paint order, which isn't
                  // reliable to control just by array order through React.
                  // bringToFront() is Leaflet's own API for this, called
                  // directly on the underlying layer so 'good' spots always
                  // sit visually above 'marginal'/'none' ones when they overlap.
                  if (layer && sc && sc.overall === 'good') layer.bringToFront();
                }}
                center={[s.lat, s.lon]}
      ...
      <div style={{ border: `1px solid ${COLORS.paperLine}`, borderRadius: 6, padding: '16px 14px 14px', marginBottom: 24, overflowX: 'auto', overflowY: 'visible' }}>
        {!activeScore ? (
          ...
          (() => {
            ...
            const roundedUp = Math.ceil(rawMax / TICK_STEP) * TICK_STEP;
            let scaleMax = Math.max(CHART_MIN_SCALE_MPH, roundedUp);
            // If the tallest bar would land within ~3mph of the top of the
            // scale, its value label has no room to render above it before
            // colliding with the direction-arrow row above the chart --
            // bump the scale up one more tick so there's always headroom.
            if (scaleMax - rawMax < 3) scaleMax += TICK_STEP;
            ...
            return (
              <>
                <div style={{ display: 'flex', gap: 1, minWidth: 940 }}>
                ...
                <div style={{ display: 'flex', gap: 1, minWidth: 940, marginTop: 4 }}>
                  <div style={{ width: AXIS_WIDTH, flexShrink: 0 }} />
                  {activeScore.scored.map((h) => (
                ...
                <div style={{ display: 'flex', gap: 6, minWidth: 940, marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 1, minWidth: 940, marginTop: 10 }}>
                  <div style={{ width: AXIS_WIDTH, flexShrink: 0 }} />
                  {activeScore.scored.map((h) => (
